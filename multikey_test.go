...
func runCrypterSubtests(t *testing.T, name string, decrypter, encrypter Crypter) {
	...
	t.Run(name, func(t *testing.T) {
		for _, text := range texts {
			...
			if len(text) == 0 {
				RequireEqual(t, string(encryptedText), "")
			}

			// decrypt
			decryptedText, err := decrypter.Decrypt(encryptedText)
			if shouldDecrypt || len(text) == 0 {
				RequireNoError(t, err)
				RequireEqual(t, decryptedText, text)
			} else {
				RequireError(t, err)
			}
		}
...
func TestMultikey(t *testing.T) {
	...
	t.Run("encrypt", func(t *testing.T) {
		...
		encryptedText, err := c1.Encrypt(text)
		RequireNoError(t, err)

		if bytes.Contains(encryptedText, text) {
			t.Fatalf("encrypted text contains plaintext")
		}
	})

	t.Run("bypass", func(t *testing.T) {
		encryptedText, err := c1bypass.Encrypt([]byte("Hello, World!"))
		RequireNoError(t, err)
